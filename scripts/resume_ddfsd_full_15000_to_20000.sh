...
: "${CKPT_PATH:?CKPT_PATH must be explicitly provided}"
: "${FREQ_STATS_PATH:?FREQ_STATS_PATH must be explicitly provided}"
...
printf '%s\n' "DATA_ROOT=${DATA_ROOT}" "RUN_ROOT=${RUN_ROOT}" "OUTPUT_PATH=${OUTPUT_PATH}" \
  ...
  "LOG_INTERVAL=${LOG_INTERVAL}" "SCHEDULER_TYPE=${SCHEDULER_TYPE}" \
...
OMP_NUM_THREADS=1 torchrun --nproc_per_node 1 --nnodes 1 train_ddfsd.py \
  ...
  --exclude_class "${EXCLUDE_CLASS}" --batch_size 16 --resume_checkpoint "${CKPT_PATH}" \
  --resume_strict_config True --total_training_steps "${TOTAL_STEPS}" --save_interval "${SAVE_INTERVAL}" \
  --eval_interval "${EVAL_INTERVAL}" --log_interval "${LOG_INTERVAL}" --num_class_train 3 \
  --num_support_train 5 --num_query_train 5 --num_support_val 10 --scheduler_type "${SCHEDULER_TYPE}" \
  --lr_scheduler_step "${LR_SCHEDULER_STEP}" --lr_scheduler_gamma "${LR_SCHEDULER_GAMMA}" \
  --rgb_backbone_lr 3e-5 --freq_backbone_lr 3e-5 --rgb_head_lr 1e-4 --freq_head_lr 1e-4 \
  --weight_decay 1e-4 --tau 0.2 --tau_r 0.1 --m_rf 1.2 --m_ff 0.6 --lambda_ff 0.5 \
  --lambda_sep_target 0.03 --lambda_sep_warmup_start 2500 --lambda_sep_warmup_end 7500 \
  --branch_dropout_dual_prob 0.90 --branch_dropout_rgb_prob 0.05 --branch_dropout_freq_prob 0.05 \
  --freq_stats_path "${FREQ_STATS_PATH}" --auto_compute_freq_stats False --use_fp16 True --pretrained False
