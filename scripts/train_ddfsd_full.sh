...
if [[ ! -d "${DATA_ROOT}" ]]; then
    ...
    echo "Set DATA_ROOT to the full GenImage root before launching this reserved full-data config." >&2
    exit 1
fi
...
OMP_NUM_THREADS=1 torchrun --nproc_per_node 1 --nnodes 1 train_ddfsd.py \
    ...
    --lambda_sep_warmup_start 10000 \
    --lambda_sep_warmup_end 30000 \
