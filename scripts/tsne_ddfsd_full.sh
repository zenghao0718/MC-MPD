...
OUTPUT_PATH=${OUTPUT_PATH:-"${RUN_ROOT}/ddfsd_full_steps200000/exclude_${EXCLUDE_CLASS}"}
...
if [[ ! -d "${DATA_ROOT}" ]]; then
    ...
    echo "Set DATA_ROOT to the full GenImage root that contains real/ ADM/ BigGAN/ glide/ Midjourney/ SD/ VQDM/." >&2
    exit 1
fi

if [[ ! -f "${CKPT_PATH}" ]]; then
    echo "Checkpoint does not exist: ${CKPT_PATH}" >&2
    echo "Set CKPT_PATH to the checkpoint for the full-data run if it uses a different step name." >&2
    exit 1
fi
