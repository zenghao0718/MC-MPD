# -*- coding: utf-8 -*-
"""Diagnostic evaluation for DDFSD: dual / rgb-only / freq-only branch modes on excluded class.

This is a standalone diagnostic script. It does not modify train_ddfsd.py or the shared
...
For a fixed (checkpoint, seed) pair, the exact same support/query split (drawn once, deterministically
from the seed) is reused across all three branch modes so that dual / rgb-only / freq-only results are
directly comparable. Support-only sigma/alpha statistics are computed once per (checkpoint, seed) and
attached to every branch-mode row (they do not depend on branch_mode, only on the support set).
"""
...
def parse_args():
    ...
    parser.add_argument("--output_dir", type=str, required=True, help="ADM experiment output dir (contains ckpt/, freq_stats.pt)")
    ...
    parser.add_argument(
        ...
        type=str,
        default="",
        help="Directory to write CSVs and logs. Defaults to <output_dir>/branch_diagnosis.",
    )
...
def main():
    ...
    out_dir = args.out_dir or os.path.join(args.output_dir, "branch_diagnosis")
        ...
        for seed in seeds:
            ...
            for branch_mode in BRANCH_MODES:
                ...
                logger.info(
                    ...
                    "sigma_rgb_mean %.6f sigma_freq_mean %.6f alpha_mean %.6f",
                    step,
                    branch_mode,
                    seed,
                    metrics["acc"],
    ...
    for step in evaluated_ckpt_steps:
        for branch_mode in BRANCH_MODES:
            ...
            alpha_mean = statistics.mean([r["alpha_mean"] for r in rows])
            alpha_min = min([r["alpha_min"] for r in rows])
            alpha_max = max([r["alpha_max"] for r in rows])
            summary_rows.append(
                {
                    "exclude_class": args.exclude_class,
                    "ckpt_step": step,
    ...
    per_seed_path = os.path.join(out_dir, "ddfsd_adm_branch_modes_per_seed.csv")
    summary_path = os.path.join(out_dir, "ddfsd_adm_branch_modes_summary.csv")
    ...
    print("\n=== DDFSD ADM branch-mode diagnosis summary ===")
    header = f"{'step':>6} | {'mode':>9} | {'ACC mean±std':>16} | {'AP mean±std':>16} | {'AUC mean±std':>16} | {'sigma_rgb':>10} | {'sigma_freq':>10} | {'alpha_mean':>10}"
    print(header)
    print("-" * len(header))
    for row in summary_rows:
        print(
            ...
            f"{row['sigma_rgb_mean']:>10.4f} | {row['sigma_freq_mean']:>10.4f} | {row['alpha_mean']:>10.4f}"
        )
    ...
    for step in evaluated_ckpt_steps:
        ...
        alpha_mean = get_metric(step, "dual", "alpha_mean")
    ...
    print(
        f"4. alpha 是否长期接近上限0.75（即长期偏向RGB）: "
        f"{alpha_near_075_count}/{total_steps} 个 checkpoint 上 alpha_mean 在 [0.73, 0.77] 区间内"
    )
